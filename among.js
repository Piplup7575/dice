...
window.among = {};
window.$t = window.among;

among.copyto = function(obj, res) {
...
among.copy = function(obj) {
    if (!obj) return obj;
    return among.copyto(obj, new obj.constructor());
}

among.element = function(name, props, place, content) {
...
among.empty = function(sel) {
    if (sel.childNodes)
        while (sel.childNodes.length)
            sel.removeChild(sel.firstChild);
}

among.remove = function(sel) {
    if (sel) {
...
among.bind = function(sel, eventname, func, bubble) {
    if (!sel) return;
...
among.unbind = function(sel, eventname, func, bubble) {
    if (eventname.constructor === Array) {
        for (var i in eventname)
...
among.one = function(sel, eventname, func, bubble) {
    var one_func = function(e) {
        func.call(this, e);
        among.unbind(sel, eventname, one_func, bubble);
    };
    among.bind(sel, eventname, one_func, bubble);
}

among.raise_event = function(sel, eventname, bubble, cancelable) {
...
among.raise = function(sel, eventname, params, bubble, cancelable) {
...
if (!document.getElementsByClassName) {
    among.get_elements_by_class = function(classes, node) {
...
among.rpc = function(params, callback, noparse) {
        ...
        promise: function() {
            return {
                then: function(callback) {
                    var deferred = among.deferred(), promise = deferred.promise();
...
among.when = function(promises) {
    var deferred = among.deferred();
